...
This demo supports both **Demo Mode** using saved sample outputs and **Live Qwen API Mode** using the Qwen Cloud API.
"""
)
...
def load_demo_result():
    ...
    if not demo_path.exists():
        st.error("Demo data file not found: data/batch_test_results.json")
        st.stop()

    results = json.loads(demo_path.read_text(encoding="utf-8"))

    if not isinstance(results, list) or len(results) == 0:
        st.error("Demo data is empty.")
        st.stop()
...
with st.sidebar:
    ...
    st.info(
        "Demo Mode uses saved outputs from data/batch_test_results.json. "
        "Live Qwen API Mode requires a valid .env file."
    )
